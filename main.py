    ...
    try:
        ...
        for key in stale_keys:
            try:
                ...
                pass  # print(f"[clear_stale_cache] Error removing {key}: {e}")
        
        if stale_keys:
            pass  # print(f"[clear_stale_cache] Cleaned {len(stale_keys)} stale cache entries")
        ...
        if os.path.exists(cache_file):
            ...
            if file_age < CACHE_DURATION:
                try:
                    ...
                    pass  # print(f"Cache read error for {cache_key}: {e}")
                    pass
    return None
...
def save_to_cache(cache_key, data):
    ...
    with cache_lock:
        try:
            ...
            pass  # print(f"Error saving to cache: {e}")
        ...
        def fetch_market_symbol(symbol):
            try:
                ...
                if df is not None and not df.empty and len(df) >= 50:
                    ...
                    pass  # print(f"[download_market_data_cache] ❌ Empty or insufficient data for {symbol}")
            except Exception as e:
                pass  # print(f"[download_market_data_cache] ❌ Error downloading {symbol}: {e}")
            return symbol, None
    ...
    try:
        ...
        try:
            ...
            if pd.isna(df['RSI']).all() or df['RSI'].var() == 0:
                pass  # print(f"[add_features_to_stock_original] {ticker}: RSI calculation produced invalid values")
        except Exception as e:
    ...
    try:
        ...
        if market_data_cache:
            if not isinstance(market_data_cache, dict) or len(market_data_cache) == 0:
                pass  # print(f"[add_features_to_stock] {ticker}: Market data cache is empty or invalid, skipping cross-asset features")
            else:
                ...
                try:
                    ...
                    pass  # print(f"[add_features_to_stock] {ticker}: Error adding cross-asset features: {e}")
                    # Continue without cross-asset features rather than failing completely
        else:
            pass  # print(f"[add_features_to_stock] {ticker}: No market data cache provided, using technical features only")
    ...
    try:
        ...
        if 'SPY' in market_data_cache:
            try:
                ...
                pass  # print(f"[add_cross_asset_features] {ticker}: Error calculating SPY features: {e}")
...
def calculate_market_regime_features(market_data_cache, target_index):
    ...
    try:
        ...
        pass  # print(f"[calculate_market_regime_features] Error: {e}")
    ...
    with ThreadPoolExecutor(max_workers=THREAD_POOL_SIZE) as executor:
        ...
        for future in as_completed(future_to_ticker):
            ...
            try:
                ...
                pass  # print(f"Error processing features for {ticker}: {e}")
...
def train_model_for_stock(ticker, df, model_ids, regime=None, regime_strength=0.5, prediction_window=5):
    ...
    try:
        ...
        if failed_features:
            pass  # print(f"[train_model_for_stock] {ticker}: Failed features ({len(failed_features)}): {failed_features[:10]}{'...' if len(failed_features) > 10 else ''}")
        ...
        if len(valid_cross_asset) == 0:
            pass  # print(f"[train_model_for_stock] {ticker}: ⚠️ NO cross-asset features found - only technical indicators will be used")
        else:
            pass  # print(f"[train_model_for_stock] {ticker}: ✅ Cross-asset features available: {valid_cross_asset[:5]}{'...' if len(valid_cross_asset) > 5 else ''}")
        ...
        if excluded_features:
            pass  # print(f"[train_model_for_stock] {ticker}: 🚫 EXCLUDED future-looking features: {excluded_features}")
        ...
        if len(final_cross_asset) == 0:
            pass  # print(f"[train_model_for_stock] {ticker}: WARNING: NO cross-asset features in final training matrix!")
        else:
            pass  # print(f"[train_model_for_stock] {ticker}: ✅ Cross-asset features in training: {final_cross_asset[:3]}{'...' if len(final_cross_asset) > 3 else ''}")
            ...
            if cross_asset_indices:
                ...
                if zero_features > 0:
                    pass  # print(f"[train_model_for_stock] {ticker}: ⚠️ {zero_features} cross-asset features are effectively zero")
                else:
                    pass  # print(f"[train_model_for_stock] {ticker}: ✅ All {len(cross_asset_indices)} cross-asset features have meaningful values")
...
def train_and_predict_model(X, y, model_id, prediction_window=1):
    ...
    try:
        ...
        if nan_count > 0:
            pass  # print(f"[train_and_predict_model] Model {model_id} removing {nan_count} NaN samples")
    ...
    with ThreadPoolExecutor(max_workers=THREAD_POOL_SIZE) as executor:
        ...
        for future in as_completed(future_to_ticker):
            ...
            try:
                ...
                pass  # print(f"Error training models for {ticker}: {e}")
...
def filter_positive_predictions(stock_predictions):
    """
    Filter stock predictions to only include positive UP predictions.
    
    This function is critical for the trading bot to ensure we only generate
    BUY signals for stocks that meet BOTH criteria:
    1. Positive predicted return (pred > 0)
    2. UP direction confirmed by CatBoost directional classifier
    
    Args:
        stock_predictions: List of dict, each containing:
            - 'ticker': str, stock symbol
            - 'pred': float, predicted return (decimal, e.g., 0.05 = 5%)
            - 'direction': str, 'up' or 'down' from CatBoost
            - 'direction_probability': float, confidence in direction
            - 'close': float, last closing price
            
    Returns:
        List of dict: Filtered predictions meeting both criteria
        
    Example:
        Input: [
            {'ticker': 'AAPL', 'pred': 0.03, 'direction': 'up'},    # KEEP
            {'ticker': 'MSFT', 'pred': -0.02, 'direction': 'down'}, # REJECT (negative)
            {'ticker': 'GOOGL', 'pred': 0.01, 'direction': 'down'}, # REJECT (wrong direction)
            {'ticker': 'AMZN', 'pred': 0.05, 'direction': 'up'}     # KEEP
        ]
        Output: [
            {'ticker': 'AAPL', 'pred': 0.03, 'direction': 'up'},
            {'ticker': 'AMZN', 'pred': 0.05, 'direction': 'up'}
        ]
    """
    positive_stocks = []
    rejected_stocks = []
    ...
    for stock in stock_predictions:
        ...
        pred_value = stock.get('pred', 0)
        direction = stock.get('direction', '')
        direction_prob = stock.get('direction_probability', 0)
        
        # Apply BOTH filters
        passes_positive_filter = pred_value > 0
        passes_direction_filter = direction == 'up'
        
        if passes_positive_filter and passes_direction_filter:
            positive_stocks.append(stock)
            # print(f"  ✅ {ticker}: ACCEPTED (pred={pred_value*100:.2f}%, "
            #       f"direction={direction}, prob={direction_prob:.1f}%)")
        else:
            rejected_stocks.append({
                'ticker': ticker,
                'reason': 'negative' if not passes_positive_filter else 'down_direction',
                'pred': pred_value,
                'direction': direction
            })
            # print(f"  ❌ {ticker}: REJECTED (pred={pred_value*100:.2f}%, "
            #       f"direction={direction}) - "
            #       f"Reason: {'Negative prediction' if not passes_positive_filter else 'Down direction'}")
    
    # Summary statistics
    total_stocks = len(stock_predictions)
    accepted_count = len(positive_stocks)
    rejected_count = len(rejected_stocks)
    acceptance_rate = (accepted_count / total_stocks * 100) if total_stocks > 0 else 0
    
    # print(f"\n📊 Filter Results:")
    # print(f"   Total stocks: {total_stocks}")
    # print(f"   ✅ Accepted: {accepted_count} ({acceptance_rate:.1f}%)")
    # print(f"   ❌ Rejected: {rejected_count} ({100-acceptance_rate:.1f}%)")
    
    # Breakdown of rejection reasons
    negative_count = sum(1 for r in rejected_stocks if r['reason'] == 'negative')
    down_direction_count = sum(1 for r in rejected_stocks if r['reason'] == 'down_direction')
    
    # print(f"   Rejection breakdown:")
    # print(f"     - Negative predictions: {negative_count}")
    # print(f"     - Down direction: {down_direction_count}")
...
def predict():
    try:
        ...
        # Trading Bot Defaults - No frontend required
        data = request.json if request.is_json else {}
        
        # HARDCODED: 18 months lookback
        default_start = (datetime.now() - timedelta(days=18*30)).strftime('%Y-%m-%d')
        
        # HARDCODED: 5-day prediction window
        prediction_window = 5
        
        # Optional parameters (can still be passed for testing)
        # For now, process one index at a time (Step 6 will loop through all 4)
        ...
        # Always use auto model selection for trading bot
        model_selection = 'auto'
        selected_models = []  # Will be set by auto selection
        
        # REMOVED PARAMETERS (no longer needed for trading bot):
        # - custom_ticker (no single ticker mode)
        # - confidence_interval (no CI calculations)
        # - is_custom (removed in Step 2)
        
        print(f"🤖 TRADING BOT MODE")
        print(f"📅 Start date: {start_date} (18 months ago)")
        print(f"📊 Index: {index}")
        ...
        # print(f"[DEBUG] {ticker} model predictions: {model_predictions}")
                ...
                if market_sentiment_score is not None:
                    ...
                    if abs(pred - original_prediction) > 0.005:  # 0.5% threshold
                        pass  # print(f"[predict] 📊 {ticker}: Sentiment adjusted prediction: {original_prediction:.6f} → {pred:.6f}")
        ...
        # **STEP 4: Filter for positive UP predictions BEFORE selecting top 2**
        print(f"\n{'='*60}")
        ...
        # Check if we have any stocks left after filtering
        if len(stock_predictions) == 0:
            print(f"\n⚠️ WARNING: No positive UP predictions found!")
            print(f"   All {original_count} stocks were filtered out.")
            print(f"   Possible reasons:")
            print(f"   - Market conditions unfavorable (bearish)")
            print(f"   - High uncertainty (low directional confidence)")
            print(f"   - Negative sentiment across all stocks")
            print(f"\n   Returning empty results for this index.")
            # Continue with empty list - will be handled by top 2 selection
        
        # Log filter results summary
        print(f"\n📈 Post-filter stock count: {len(stock_predictions)}")
        if len(stock_predictions) > 0:
            avg_pred = np.mean([s['pred'] for s in stock_predictions])
            max_pred = max([s['pred'] for s in stock_predictions])
            min_pred = min([s['pred'] for s in stock_predictions])
            print(f"   Average prediction: {avg_pred*100:.2f}%")
            print(f"   Range: {min_pred*100:.2f}% to {max_pred*100:.2f}%")
    ...
    with ThreadPoolExecutor(max_workers=THREAD_POOL_SIZE) as executor:
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                pass  # print(f"Error calculating direction confidence for {ticker}: {e}")
    
    # print(f"✅ Added directional confidence to {len(stock_predictions)} predictions")
    return stock_predictions

# ============================================================================
# NOTE: Flask server removed - use trading_bot.py for standalone execution
# ============================================================================
