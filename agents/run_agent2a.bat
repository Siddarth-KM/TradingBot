...
if not defined CLAUDE_BIN (
    echo [%date% %time%] ERROR: claude.exe not found under .vscode\extensions\anthropic.claude-code-* >> logs\excel.log
    exit /b 2
)
"%CLAUDE_BIN%" -p "Read agents/excel_writer.md and execute it in MODE A" --allowedTools "Bash,Read" >> logs\excel.log 2>&1
